        ...
        <section className={`bg-gray-200 opacity-75 border-t-4 border-${colour}-500 text-black rounded-lg min-h-[400px] grid grid-cols-4`}>
          ...
          <section className="col-span-3 row-span-3 grid grid-cols-3 grid-rows-3">
            {["7", "8", "9", "4", "5", "6", "1", "2", "3"].map((num) => (
              <button key={num} id={num} className={`hover:bg-${colour}-300`} onClick={() => handleEntry(event.target.textContent)}>{num}</button>
            ))}
          </section>
          ...
          <button id="0" className={`hover:bg-${colour}-300`} onClick={() => handleEntry(event.target.textContent)}>0</button>
